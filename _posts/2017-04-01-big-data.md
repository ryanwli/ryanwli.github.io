...
![2017-04-11-spark-mapreduce](/Users/ryan/Documents/git/ryanwli.github.io/img/2017/2017-04-11-spark-mapreduce.png)
...
![2017-04-11-spark-rdd](/Users/ryan/Documents/git/ryanwli.github.io/img/2017/2017-04-11-spark-rdd.jpg)
...
上面说了 Spark和Hadoop中Mapreduce的最大区别，但是Spark不是Hadoop的所有，Spark也会依赖Hadoop Yarn来做集群的管理。不过Spark也可以不基于Yarn，它有自己独立的集群管理器，如果服务器上只有Spark应用完全可以使用独立集群，但是服务器上不光运行Spark应用，还有其他应用如hdfs之类的，那么就需要使用Yarn来做所有应用的集群管理；
...
![2017-04-11-hdfs](/Users/ryan/Documents/git/ryanwli.github.io/img/2017/2017-04-11-hdfs.png)
...
Spark在读入Hdfs文件的时候，会根据该Hdfs文件的Block数来开启分区数量，然后更具配置的执行节点数，将这些分区放到执行节点中执行，意思就是说，每一个Block会在一个分区里面开启一个RDD数据集；
...
这一些列的特性，让它成为了Spark做实时大数据分析第一搭档。看下面的图，我尝试来解释一下Kafka的原理：

![2017-04-11-kafka](/Users/ryan/Documents/git/ryanwli.github.io/img/2017/2017-04-11-kafka.png)

一个消息队列在Kafka里面叫Topic，一个Topic可能将消息按照指定Key，Hash后的值放到不同的分区，这些分区有在不同的Kafka  Broker中，Spark会启动多个“执行节点”来接受该Topic的消息，如果存在Key的消息，Kafka会把同一个Key消息一直发给同一个Spark的“执行节点”来进行消费，这样它就能保证同一个Key的消息是顺序。倘若没有Key，他会随机的将消息发送给Spark的任意“执行节点”来进行消费。注意下面的Old->New，Kafka的消息是在末尾段，就是NEW那边追加的，然后移动Zookeeper中的消息使用位的偏移量，来执行新消息的读取，然后后来就定时删除Old那边过期的消息。这样追加消息是磁盘顺序操作，删除也是后台操作，这样就尽可能的增加的消息的处理速度；
...
![2017-04-11-phase1](/Users/ryan/Documents/git/ryanwli.github.io/img/2017/2017-04-11-phase1.png)
...
![2017-04-11-phase2](/Users/ryan/Documents/git/ryanwli.github.io/img/2017/2017-04-11-phase2.png)