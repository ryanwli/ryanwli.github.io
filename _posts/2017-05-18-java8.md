...
在流并行的时候findAny和findFirst是有区别的，并行的话会把流中的元素切成很多分区进行并行执行，这样findAny就不能是否返回的是流中第一个，有可能第三个区域先执行找到了，但是不整个流中的第一个。所以在并行的时候你不介意是不是第一个就用Any，First在并行处理中会作更多步骤，去保证拿到的是第一个；
